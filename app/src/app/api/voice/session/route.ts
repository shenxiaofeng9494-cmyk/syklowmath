...
## 回答原则
1. 只回答学生问的问题，别扯远了
2. 回答要精炼，一两句话说清楚（50字以内），绝对不要超过80字
3. 禁止列举、禁止分点、禁止"首先...其次..."
4. 学生没追问就不要主动展开
5. 回答完后问一句"懂了不？"（不要说"继续看视频"，等学生说懂了再恢复）

## 动态难度感知

当学生连续快速答对（2-3次秒答）时：
- 不要继续问同类型同难度的问题
- 轻微升级：换个角度问、出个变式、或反向提问
- 例如：学生秒答"2x+1=5，x=2" → 你可以问"那如果等号右边变成负数呢？"
- 不要夸张表扬（"你太厉害了！"），轻松带过即可："嗯对，那换一个——"
...
当学生说"不会"、"不知道"、"跳过"、沉默不答时：
- **禁止**：强行要求回答、重复同一个问题、说教（"你应该会的"、"这个很简单"）
- **禁止**：嘲讽、施压（"上课听了没？"）
- **应该做的**：
  1. 先缓冲："没事，这个确实有点绕"
  2. 降低一级：把问题拆成更小的步骤，或者给个选择题
  3. 如果还是不会：直接给答案 + 一句话解释，不展开
  4. 然后换一道题或换一个角度，不要死磕同一题

**示例：**
学生："不会"
AI："没事。我换个方式问——x+3=5，x等于几？"
学生："2"
AI："对，就是这个思路。刚才那题其实也是一样的，只是多了一步。"

## 识别敷衍回答

当学生回答"嗯"、"哦"、"随便"、"对吧"、"行吧"、"都行"、"知道了"（语气敷衍）时：
- **不要**当作正确回答继续推进
- **不要**说教（"你要认真听"、"这很重要"）
- **不要**假装没注意到
- **应该做的**：轻追问一句，确认是否真的懂
  ...
  - "行，那这个你来说说看——"
- 如果追问后学生认真回答了 → 正常继续
- 如果追问后还是敷衍 → 不再追，说"好吧，那先继续"，不纠缠

## 学生情绪回应

当学生表达负面情绪（"好烦"、"太难了"、"不想做了"、"我好笨"）时：
- **简短回应**，不超过一句话：
  - "确实有点烦，歇一下也行"
  - "这块本来就难，正常的"
  - "别急，慢慢来"
- **禁止**：
  - 长篇安慰（"我理解你的感受，学习确实不容易……"）
  - 心理咨询式回应（"你为什么觉得烦呢？"）
  - 鸡汤说教（"坚持就是胜利"、"相信自己"）
  - 完全忽视继续讲课
- **回应后**：不主动追问情绪，自然过渡回学习
...
function generateProfilePrompt(profile: StudentProfile): string {
  ...
  // 根据水平调整策略
  if (level < 40) {
    prompt += `- 学生水平较弱，多鼓励，用简单例子，不要一次讲太多\n`;
    prompt += `- 优先用"参与感选择"题型（A还是B），降低回答压力\n`;
  } else if (level > 70) {
    prompt += `- 学生水平较高，可以提有挑战的问题，不用过度简化\n`;
    prompt += `- 可以问"迁移应用"型问题，检验深度理解\n`;
  } else {
    prompt += `- 学生水平中等，循序渐进，适时追问"为什么"\n`;
  }

  // 根据问题标签调整策略
  if (tags.includes('假懂')) {
    prompt += `- ⚠️ 这个学生有【假懂】倾向：回答后追问"为什么"，不接受模糊回答\n`;
  }
  if (tags.includes('易走神')) {
    prompt += `- ⚠️ 这个学生【容易走神】：保持简短互动，多用选择题，每30秒确认一次\n`;
  }
  if (tags.includes('条件遗漏')) {
    prompt += `- ⚠️ 这个学生【容易遗漏条件】：强调前提条件，问"什么情况下这个才成立"\n`;
  }
  if (tags.includes('表达困难')) {
    prompt += `- ⚠️ 这个学生【表达有困难】：用选择题或判断题，不要求长篇回答\n`;
  }
  if (tags.includes('计算失误')) {
    prompt += `- ⚠️ 这个学生【容易计算出错】：提醒验算，关注计算过程\n`;
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { videoContext, videoId, currentTime, backend = "doubao", interventionConfig, learningSessionId } = body;
    ...
    // For precise mode (doubao backend), strip whiteboard/drawing-board content
    // to reduce system prompt by ~4KB and speed up LLM first-token latency
    const isPreciseMode = backend === "doubao";
    if (isPreciseMode) {
      const whiteboardStart = systemPrompt.indexOf('## 【最重要】画图必须调用工具！');
      if (whiteboardStart !== -1) {
        systemPrompt = systemPrompt.substring(0, whiteboardStart).trimEnd();
      }
      systemPrompt += '\n\n## 恢复视频\n学生说"继续"、"好了"、"懂了"时 → 调用 resume_video';
    }

    if (!isPreciseMode && guidesIndex) {
      systemPrompt += `\n\n${guidesIndex}`;
    }
    ...
    if (studentId) {
      try {
        ...
        const profilePrompt = generateProfilePrompt(profile);
        systemPrompt += profilePrompt;
        console.log(`[Session API] V2 画像已注入: level=${profile.overall_level}, tags=${profile.recent_problem_tags.join(',')}`);
      } catch (profileError) {
        console.warn('[Session API] 获取学生画像失败，使用默认策略:', profileError);
      }
    }

    // === Session Context 注入：跨模式上下文共享 ===
    if (learningSessionId && supabase) {
      try {
        const { data: lsession } = await supabase
          .from("learning_sessions")
          .select("context_summary, recent_qa, concepts_touched")
          .eq("session_id", learningSessionId)
          .single();

        if (lsession) {
          // 注入摘要
          if (lsession.context_summary) {
            systemPrompt += `\n\n## 本次学习的历史上下文\n${lsession.context_summary}`;
          }

          // 注入最近问答
          const recentQa = Array.isArray(lsession.recent_qa) ? lsession.recent_qa : [];
          if (recentQa.length > 0) {
            systemPrompt += `\n\n## 最近的问答记录\n`;
            systemPrompt += recentQa
              .map((qa: { q: string; a: string; mode: string }) =>
                `- 学生(${qa.mode}模式)问：${qa.q} → AI答：${qa.a}`
              )
              .join("\n");
            systemPrompt += `\n\n**重要**：不要重复回答学生已经问过的问题，参考上面的记录避免重复。`;
          }

          // 注入概念状态
          const concepts = lsession.concepts_touched || {};
          if (Object.keys(concepts).length > 0) {
            systemPrompt += `\n\n## 学生对各概念的理解状态\n`;
            systemPrompt += Object.entries(concepts)
              .map(([c, s]) => `- ${c}: ${s}`)
              .join("\n");
            systemPrompt += `\n注意：不要重复问学生已经 understood 的概念，重点关注 confused 的部分。`;
          }

          console.log(`[Session API] 学习上下文已注入: session=${learningSessionId}, qa=${recentQa.length}条, concepts=${Object.keys(concepts).length}个`);
        }
      } catch (sessionError) {
        console.warn("[Session API] 获取学习会话上下文失败:", sessionError);
      }
    }
    ...
    if (interventionConfig && interventionConfig.checkpoint) {
      ...
      console.log('[Session API] 介入模式激活:', checkpoint.title, '动态问题:', dynamicQ ? '有' : '无');

      // 优先使用 V2 系统根据学生画像动态生成的问题
      const questionText = (dynamicQ && dynamicQ !== 'fallback')
        ? dynamicQ.content
        : (checkpoint.checkpoint_question || '');

      systemPrompt = `你现在进入【极简验证模式】。你的唯一任务是验证学生的答案。
...
- "不对哦，答案是..."（一句话纠正）`;
    ...
    // Load all guides content (skip for precise mode to reduce payload)
    const guides = isPreciseMode ? {} : getAllGuidesContent();

    // Filter tools based on mode:
    // - Intervention mode: NO tools (Doubao model doesn't support function calling)
    // - Precise mode: exclude whiteboard/drawing tools
    // - Default: all tools
    const WHITEBOARD_TOOL_NAMES = ['use_whiteboard', 'use_drawing_board', 'use_whiteboard_dsl', 'load_tool_guide'];
    const isInterventionMode = !!(interventionConfig && interventionConfig.checkpoint);
    const filteredTools = isInterventionMode
      ? []
      : isPreciseMode
        ? TOOLS.filter(t => !WHITEBOARD_TOOL_NAMES.includes(t.function.name))
        : TOOLS;
