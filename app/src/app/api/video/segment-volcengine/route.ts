  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Step 2: 创建或更新视频记录
        sendProgress('database', 10, '更新视频记录...')
        ...
        // Step 3: 调用火山引擎场景切分
        sendProgress('segmentation', 15, '正在调用火山引擎场景切分...')
        ...
        // Step 5: 转换为节点格式
        sendProgress('convert', 60, '转换节点数据...')
        ...
        for (let i = 0; i < nodeDataList.length; i++) {
          ...
          const progressPercent = 70 + Math.round((i / totalNodes) * 25)
...
export async function GET() {
  return NextResponse.json({
    configured: isVolcengineConfigured(),
    message: isVolcengineConfigured()
      ? '火山引擎已配置'
      : '火山引擎未配置，请设置环境变量',
  })
}
